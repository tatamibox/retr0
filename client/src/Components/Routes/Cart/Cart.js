...
function Cart() {
    ...
    useEffect(() => {
        if (cartId) {
            setLoading(true)
            axios.post('https://retr0-server.vercel.app/getCartContents', { cartId })
                .then((res) => {
                    console.log(res.data)
                    setLoading(false)
    ...
    useEffect(() => {
        ...
        if (cartProductIds) {
            setLoading(true)
            axios.post('https://retr0-server.vercel.app/getMultipleProducts', { cartProductIds })
                .then((res) => {
                    console.log(res.data)
                    setLoading(false)
    ...
    const removeHandler = (i) => {
        setLoading(true)
        axios.put('https://retr0-server.vercel.app/removeCartItem', {
            cartId: cartId,
            index: i,
